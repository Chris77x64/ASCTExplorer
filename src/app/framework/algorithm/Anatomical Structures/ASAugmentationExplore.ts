...
export class ASAugmentationExplore {

  bends: Bend[]
  bendCount: number

  depth : number
  exploreThreshhold : number

  constructor( graph, bfsThreshold) {

    this.bends = []
    this.bendCount = -1;
    this.depth = Number.NEGATIVE_INFINITY
    this.exploreThreshhold = bfsThreshold+1
      ...
      currentLayer.forEach(vertexID =>{
        ...
        if( ! (vertexID in tree.outDegree)) {
          ...
          if (vertexID in graph.inDegree) {
            ...
            for (let j = i + 1; j < Math.min(layers.length,i+this.exploreThreshhold); j++) {
