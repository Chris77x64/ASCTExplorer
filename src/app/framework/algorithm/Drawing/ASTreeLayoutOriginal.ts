...
export class ASTreeLayoutOriginal {
    ...
    drawLayout(currentID, recursionDepth, graph, tree, drawing){
        ...
        if( currentID in tree.outDegree) {
            ...
            if ( children.length == 1){
                currentY = Math.max(drawing[children[0]].y,this.nextYonLayer[recursionDepth]+2*this.offsetIncrementAS)
            }
            else{
                ...
                currentY = Math.max((leftmostY+rightmostY) / 2,this.nextYonLayer[recursionDepth]+2*this.offsetIncrementAS)
            }
        }
        else{
            currentY = this.nextYonLayer[recursionDepth]+2*this.offsetIncrementAS
        }
        drawing[currentID] = new Point2D( currentX,currentY)
        this.nextYonLayer[recursionDepth] = this.nextYonLayer[recursionDepth] + 2*this.offsetIncrementAS
